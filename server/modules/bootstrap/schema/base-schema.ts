...
CREATE TABLE IF NOT EXISTS agents (
  ...
  sprite_config TEXT DEFAULT NULL,
  mood TEXT NOT NULL DEFAULT 'happy' CHECK(mood IN ('happy','focused','tired','frustrated','excited','curious')),
...
-- Chao Empire: Room assignments (which room each agent is in)
CREATE TABLE IF NOT EXISTS room_assignments (
  agent_id TEXT PRIMARY KEY REFERENCES agents(id) ON DELETE CASCADE,
  room TEXT NOT NULL DEFAULT 'desk' CHECK(room IN ('desk','meeting','gym','library','break','server','ceo','hallway')),
  entered_at INTEGER NOT NULL DEFAULT (unixepoch()*1000)
);
...
CREATE INDEX IF NOT EXISTS idx_agent_chats_agents ON agent_chats(from_agent_id, to_agent_id, created_at DESC);

-- Chao Empire: Model provider health tracking
