...
export function useAppLabels({
  ...
  const loadingTitle = pickLang(uiLanguage, {
    ko: "Chao Empire 로딩 중...",
    en: "Loading Chao Empire...",
    ja: "Chao Empireを読み込み中...",
    zh: "Chao Empire 加载中...",
  });
  ...
  const effectiveUpdateStatus = forceUpdateBanner
    ? {
        ...
        release_url: updateStatus?.release_url ?? "https://github.com/natebag/chao-empire/releases/latest",
        checked_at: Date.now(),
        enabled: true,
        repo: updateStatus?.repo ?? "natebag/chao-empire",
        error: null,
      }
    : updateStatus;
  ...
  const updateReleaseUrl =
    ...
    `https://github.com/${effectiveUpdateStatus?.repo ?? "natebag/chao-empire"}/releases/latest`;
